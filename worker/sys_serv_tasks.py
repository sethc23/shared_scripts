...
def pgsql_query(qry=None):
    return pd_read_sql(qry,sys_eng).to_json()

@task
def sn4_backup():
    cmds                                    =   ['cd ~/.android',
                                                 'mkdir `date +%Y_%m_%d`',
                                                 'scp -r sn4:/sdcard/.backups/* ./`date +%Y_%m_%d` > /dev/null 2>&1',
                                                 'retar sn4_backup_`date +%Y.%m.%d`.gz ./`date +%Y_%m_%d`',
                                                 'rm -fr `date +%Y_%m_%d`',
                                                 'scp -r sn4:/sdcard/MDScan3/.Projects/* ./scans/  > /dev/null 2>&1',
                                                 'sn4_ar_msg ssh_droid_stop']
    cmd                                     =   "bash -l -c '%s'" % ';'.join(cmds)
